...
#### 20210531(월) 작업.
- ERD기준으로 게시판UI 수정합니다.
- 관리자단 AdminLTE 적용 - 스프링시간|UI디자인

#### 20210528(금) 작업.
- UI트렌드 용어: 카드(트럼프)UI, 뱃지(명찰), 리본
- 소스 인스펙트(소스분석);
- 데이터를 전송할때 사용 문장=쿼리스트링(QueryString)의 시작(?) :  ?키=value&키=value
- 뒤로가기 허용하면, 게시물 테러가 가능하다. (새로고침하면 계속 등록되기 때문)
- 뒤로가기를 허용하지 않게 페이지를 이동하는 방식(JS에서는 location.replace)
- 관리자단 admin/borad_write.html, board_view.html
- 대시보드 home.html(최근 회원가입정보, 최근 게시물 정보)
- 관리자단 AdminLTE적용 -스프링시간 |UI디자인 시간 선택 후 아래 작업진행혜정
- (회원관리 CRUD-html, 게시판생성관리CRUD-html)
- 이클립스로 작업한 html 내용을 ->resources폴더(admin,home,root파일까지)로 배치
- 스프링 작업의 시작
...
#### 20210526(수) 작업.
- 검색엔진에 최적화 : 의미있는 태그를 사용했는지. <header>,<section>,<footer>,<article> html5부터 만들어진 태그를 사용하면, 구글검색에 잘 노출된다.
- 유효성검사(validation) 입력값이 제대로된 값인지 확인하는 과정.
- html5에는 내장된 유효성검사가 있다. required(공백체크), type='email'(이메일 형식 체크), type='password'(입력값을 숨김처리)
,type="number"(숫자체크)
- js로 유효성검사 비밀번호 확인(2개 입력값을 체크)
- 반응형 서브페이지들(로그인, 회원가입, 마이페이지).
- HTML5부터는 유효성 검사가 내장되어있기 때문에 JS로 빈칸인지 코딩할 필요가 없음. required 속성을 태그에 집어넣으면 끝.
- 기획시, 메인1 CSS 네이밍, 서브1 CSS 네이밍(과장/이사급이 ui2개를 만든다 or 테마포레스트 같은 디자인UIhtml+css+js를 구매해서 사용.) ->퍼블리셔, 프론트개발자 CSS나 HTML작업을 이어서 함.
- 금액이 4000이상~2억 안팎인 프로젝트는 자체UI를 사용.


#### 20210525(화) 작업.
- css: 계층:조부 > 부 > 본인(timnline) > 자식(collapse) > 손자(time-label) > 증손자
- board_view.html 댓글 AdminLTE3의 페이징 UI 추가 OK
